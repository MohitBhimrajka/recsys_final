...
const TABS_CONFIG: {
  ...
  {
    id: "ensemble" as TabId,
    label: "Combined Suggestion",
  ...
  },
  {
    ...
    icon: <FiSliders size={16} />,
    order: 6,
    modelInfoName: "ALS (f=100)",
  },
  {
    id: "popularity" as TabId,
    label: "Popularity",
    icon: <FiInbox size={16} />,
    order: 7,
    modelInfoName: "Popularity",
  },
...
function cardDisplayReducer(
  ...
  switch (action.type) {
    case "HIDE":
      return {
        ...state,
        hiddenCards: new Set(state.hiddenCards).add(action.payload),
      };
...
const Tab: React.FC<TabProps> = ({
  ...
  const baseClasses =
    "relative flex-shrink-0 flex items-center gap-2 px-4 sm:px-5 py-3 text-sm font-medium transition-colors duration-200 outline-none focus-visible:ring-1 focus-visible:ring-primary rounded-t-md";
  ...
  const inactiveCombinedClasses = `${inactiveBaseClasses} ${
    highlight && !isActive ? inactiveHighlightClasses : inactiveNormalClasses
  }`;
  return (
    <motion.button
      onClick={onClick}
      ...
      className={`${baseClasses} ${
        isActive ? activeClasses : inactiveCombinedClasses
      }`}
...
const ScoreDistributionInfo: React.FC<{ scores: number[] }> = ({ scores }) => {
  ...
  return (
    <div className="mt-6 text-center text-xs text-text-muted border-t border-border-color/50 pt-4">
      {" "}
      Top {scores.length} Scores - Min:{" "}
      <strong className="text-text-secondary">{minScore}</strong> | Avg:{" "}
      <strong className="text-text-secondary">{avgScore}</strong> | Max:{" "}
      <strong className="text-text-secondary">{maxScore}</strong>{" "}
    </div>
  );
};
...
const DemoPage: React.FC = () => {
  ...
  const [ensembleRecommendations, setEnsembleRecommendations] = useState<
    RecommendationItem[] | null
  >(null);
  const [allModelRecommendations, setAllModelRecommendations] =
    useState<AllModelsRecs | null>(null);
  ...
  const [currentPresentationDetail, setCurrentPresentationDetail] =
    useState<PresentationDetailInfo | null>(null);
  ...
  const openDetailModal = (presentation: PresentationDetailInfo) => {
    setCurrentPresentationDetail(presentation);
  ...
  const handleUserSelect = useCallback(
    async (userId: number | null, scrollIntoView: boolean = false) => {
      ...
      try {
        ...
        const [ensembleResult, allModelsResult] = await Promise.allSettled([
          fetchRecommendations(numericUserId, 9),
          fetchAllModelRecommendations(numericUserId, 9),
        ]);
        let fetchError: string | null = null;
        if (ensembleResult.status === "fulfilled")
          setEnsembleRecommendations(ensembleResult.value);
        else {
          console.error("Failed ensemble:", ensembleResult.reason);
          fetchError = `Failed combined suggestions: ${
            ensembleResult.reason?.message || "Unknown error"
          }. `;
        }
        if (allModelsResult.status === "fulfilled")
          setAllModelRecommendations(allModelsResult.value);
        else {
          console.error("Failed all models:", allModelsResult.reason);
          fetchError =
            (fetchError || "") +
            `Failed individual model results: ${
              allModelsResult.reason?.message || "Unknown error"
            }.`;
        }
        if (fetchError) setError(fetchError + " Please check the API server.");
        else {
          setError(null);
  ...
  const handleRandomUser = useCallback(async () => {
    ...
    try {
      ...
      if (randomUser) {
        ...
        if (userSelectorRef.current?.select)
          userSelectorRef.current.select.setValue(
            { value: randomId, label: String(randomId) },
            "select-option"
          );
        setTimeout(() => handleUserSelect(randomId, true), 50);
      } else throw new Error("API did not return a random user.");
    } catch (err: unknown) {
      console.error("Random user fetch failed:", err);
      setError(
        `Error selecting random user: ${
          err instanceof Error ? err.message : "Unknown error"
        }.`
      );
      setSelectedUserId(null);
      if (userSelectorRef.current?.select)
        userSelectorRef.current.select.clearValue();
    } finally {
  ...
  const handleCardClick = (recommendation: RecommendationItem) =>
    openDetailModal(recommendation);
  ...
  const contextSectionVariants = {
    ...
    }, // Smooth ease-out for collapse
    visible: {
      height: "auto",
      opacity: 1,
      marginTop: "1rem",
      transition: { duration: 0.4, ease: [0.4, 0, 0.2, 1] },
    }, // Smooth ease-out for expand
  };
    ...
    >
      ...
      >
        ...
        <p className="text-text-secondary md:text-lg max-w-3xl mx-auto">
          {" "}
          Select a student ID (or try random) to view recommendations based on
          the <strong className="text-primary">OULAD dataset</strong>. Explore
          combined suggestions and compare individual model results.{" "}
        </p>
      </motion.header>
      {/* Control Section */}
      <motion.div
        className="max-w-lg mx-auto mb-12 md:mb-16 px-4"
        variants={itemVariant}
      >
        <div className="bg-surface rounded-xl shadow-xl border border-border-color p-6">
          ...
          <UserSelector
            ref={userSelectorRef}
            onUserSelect={(userId) => handleUserSelect(userId, true)}
          ...
          >
            ...
            <FiRefreshCw
              size={16}
              className={`${isFetchingRandom ? "animate-spin" : ""}`}
            />{" "}
      ...
      <motion.div className="max-w-4xl mx-auto mb-12 md:mb-16 px-4" layout>
        {" "}
        {/* Add layout for smooth reflow */}
        <motion.button
        ...
        >
          ...
          <motion.div
            animate={{ rotate: isContextVisible ? 180 : 0 }}
              ...
              <div className="mt-4 p-5 bg-surface/30 border border-border-color/50 rounded-lg text-sm text-text-muted space-y-3 shadow-inner">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <span>
                    ...
                    Recommendations stem from the{" "}
                    <a
                      href="https://www.kaggle.com/datasets/anlgrbz/student-demographics-online-education-dataoulad"
                      target="_blank"
                    ...
                    , analyzing anonymized student engagement (clicks) in the
                ...
                <div className="flex items-start gap-2">
                  ...
                  <span>
                    ...
                    We suggest specific course 'presentations' (e.g.,{" "}
                    <code className="text-xs px-1 py-0.5 bg-background/50 rounded">
                      AAA_2013J
                    </code>
                    ), representing a unique offering of a course module in a
                    semester.
                  </span>
                </div>
                <div className="flex items-start gap-2 p-3 bg-yellow-900/20 border border-yellow-700/30 rounded-md">
                  {" "}
                  {/* Highlighted Note */}
                  <FiAlertTriangle
                    size={18}
                  ...
                  <span className="text-yellow-200">
                    <strong className="text-yellow-100">
                      Crucial Context (Filtering Impact):
                    </strong>{" "}
                    To ensure model quality, data was filtered based on
                    interaction counts. This demo's models were trained on only{" "}
                    <strong className="underline decoration-dotted">
                      22 unique course presentations
                    </strong>
                    . This significantly limits the diversity of recommendations
                    you'll see here.
                  </span>
                </div>
                <div className="flex items-start gap-2">
                  ...
                  <span>
                    ...
                    <ul className="list-disc list-inside ml-4 mt-1 text-xs space-y-1">
                      <li>
                        ...
                        Our best overall guess, using a weighted average of
                        multiple models.
                      </li>
                      <li>
                        <strong className="text-text-secondary">
                        ...
                        Tools to directly compare different models' outputs.
                      </li>
                      <li>
                        <strong className="text-text-secondary">
                          Individual Models (ItemCF, NCF, etc.):
                        </strong>{" "}
                        Raw rankings from each specific algorithm.
                      </li>
                    </ul>
                  </span>
                </div>
      ...
      <div ref={resultsRef} className="min-h-[400px]">
        <AnimatePresence mode="wait">
          ...
          {isLoading && selectedUserId && (
            ...
            >
              {" "}
              <h2 className="text-xl font-semibold mb-10 text-center text-text-muted animate-pulse">
                {" "}
                Generating Recommendations for Student{" "}
                <span className="font-bold text-primary">{selectedUserId}</span>
                ...{" "}
              </h2>{" "}
              <div className="border-b border-border-color mb-8 flex justify-center">
                {" "}
                <div className="animate-pulse bg-surface/80 h-12 w-48 rounded-t-lg mr-1"></div>{" "}
                <div className="animate-pulse bg-surface/80 h-12 w-48 rounded-t-lg ml-1"></div>{" "}
                <div className="animate-pulse bg-surface/80 h-12 w-48 rounded-t-lg ml-1"></div>{" "}
              </div>{" "}
          ...
          {!isLoading && !error && !selectedUserId && (
            <motion.div
              ...
              animate="visible"
              exit="exit"
              className="text-center text-text-muted pt-10 pb-10 min-h-[300px] px-4"
            >
              {" "}
              <motion.div
                initial={{ y: -5 }}
              ...
              >
                ...
                <FiArrowDownCircle
                  size={48}
                  className="mx-auto mb-5 text-border-color opacity-60"
                />{" "}
              </motion.div>{" "}
              ...
              <p className="text-sm mt-1">
                Use the search or random button above.
              </p>{" "}
            </motion.div>
          )}
          ...
          {!isLoading && !error && selectedUserId && (
            ...
            >
              {/* Tab Buttons */}
              <div className="border-b border-border-color mb-8 md:mb-12 px-4 overflow-x-auto sticky top-16 bg-background/80 backdrop-blur-sm z-40">
                {" "}
                {/* Made tabs sticky */}
                <nav
                  className="-mb-px flex justify-start sm:justify-center space-x-1 sm:space-x-3"
                  aria-label="Tabs"
                >
                  {TABS_CONFIG.map((tab) => (
                    <Tab
                        ...
                        >
                            ...
                            <div className="space-y-8 md:space-y-10">
                              <div className="text-center">
                                ...
                                <h2 className="text-2xl font-semibold mb-2 text-text-primary">
                                  Combined Suggestion (Ensemble)
                                </h2>{" "}
                                <p className="text-sm text-text-muted max-w-xl mx-auto">
                                  {" "}
                                  Top suggestions based on a weighted average of
                                  scores from all models. Interact with cards
                                  below (hide/highlight).{" "}
                                </p>{" "}
                              </div>
                              <RecommendationList
                          ...
                          {[
                            ...
                            (() => {
                              ...
                              const modelRecs = allModelRecommendations
                                ? allModelRecommendations[modelKey]
                                : [];
                              return (
                                <div className="space-y-8 md:space-y-10">
                                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-1">
                                    <button
                                      onClick={() =>
                                        modelInfo &&
                                        openModelInfoModal(modelInfo.name)
                                      }
                                      disabled={!modelInfo}
                                      className="text-xl md:text-2xl font-semibold text-text-primary flex items-center gap-2 hover:text-primary disabled:hover:text-text-primary disabled:opacity-60 transition-colors outline-none focus-visible:ring-1 focus-visible:ring-primary rounded text-left"
                                      title={
                                        modelInfo
                                          ? `Learn about the ${modelInfo.name} model`
                                          : tab.label
                                      }
                                    >
                                      {" "}
                                    ...
                                    {modelInfo?.evaluationScore && (
                                      <p className="text-xs text-left sm:text-right text-text-muted italic whitespace-nowrap">
                                        Offline NDCG@10:{" "}
                                        <span className="font-medium text-text-secondary">
                                          {modelInfo.evaluationScore.toFixed(4)}
                                        </span>
                                      </p>
                                    )}
                                  </div>
                                  {modelInfo?.description && (
                                    <p className="text-sm text-text-muted mb-6">
                                      {modelInfo.description.split(".")[0]}.
                                    </p>
                                  )}
                                  <RecommendationList
                                    recommendations={modelRecs || []}
                                    selectedUserId={selectedUserId}
                                    onCardClick={handleCardClick}
                                    isIndividualModelList={true}
                                    modelName={modelKey}
                                  />
                                </div>
                              );
                          ...
                          {tab.id === "analysis" && (
                            <AnalysisDashboard
                              ...
                              modelColors={modelInfos.reduce(
                                ...
                                { Combined: "var(--color-primary-default)" }
                              )}
                            />
                          )}
                        </motion.section>
      ...
      <PresentationDetailModal
        ...
        presentation={currentPresentationDetail}
      />
    </motion.div>
  );
};
